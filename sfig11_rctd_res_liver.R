...
lapply(c("E9.5", "E11.5", "E13.5"), \(stg) {
  ...
  pie_p <- ggplot2::ggplot() +
    scatterpie::geom_scatterpie(
      ...
      pie_scale = scl,
      col = NA,
      data = hcc2t_ptn_ratio,
      cols = colnames(hcc2t_ptn_ratio)[-c(1:3)],
      long_format = F
    ) +
  ...
  ggsave(
    glue("sfig12a_rctd_prop_pie_{stg}.pdf"),
    pie_p,
    width = plot_width,
    height = plot_height
  )
...
pie_p <- ggplot2::ggplot() +
  ...
  theme(legend.key.size = unit(3, "mm"),
        legend.position = "right")
ggsave(
  glue("sfig12b_liver_rctd_prop_pie.pdf"),
  pie_p,
  width = 4,
  height = 3
)
...
pdf("sfig12c_rctd_proportion_spatial_feats.pdf",
    height = 2,
    width = 8)
print(p_lst1)
dev.off()
