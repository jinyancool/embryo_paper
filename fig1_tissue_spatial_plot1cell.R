...
plst1 <- lapply(samples_mmu, \(samp) {
  ...
  ggsave(
    glue("fig1b_mouse_tissuetype_spatial_{samp}.pdf"),
    dim1,
    width = plot_width,
    height = plot_height
  )
...
plst1[["legend"]] <- cowplot::plot_grid(NULL,
                                        legend,
                                        NULL,
                                        ncol = 3,
                                        rel_widths = c(.1, 1, .1))
ggsave(
  "fig1b_legend_mouse_tissuetype_spatial.pdf",
  plst1[["legend"]],
  width = 2,
  height = 2
)
### combination -----
fig1b_mouse <- cowplot::plot_grid(
  plotlist = c(plst1),
  nrow = 1,
  rel_widths = c(.8, 1, 1.2, 1)
)
ggsave(
  "fig1b_mouse_tissuetype.pdf",
  fig1b_mouse,
  width = unit(8, "cm"),
  height = unit(3, "cm")
)
...
plst1 <- lapply(samples_hsa, \(samp) {
  ...
  dim1 = Seurat::SpatialDimPlot(
    ...
    Seurat::NoAxes() & Seurat::NoLegend()
  ggsave(
    glue("fig1c_human_tissue_spatial_{samp}.pdf"),
    dim1,
    width = plot_width,
    height = plot_height
  )
...
plst1[["legend"]] <- cowplot::plot_grid(NULL,
                                        ...
                                        rel_widths = c(.1, 1, .1))
ggsave(
  "fig1c_legend_human_tissue_spatial.pdf",
  plst1[["legend"]],
  width = 2,
  height = 2
)
### combination -----
fig1c_human <- cowplot::plot_grid(
  plotlist = c(plst1),
  nrow = 1,
  rel_widths = c(.6, 1, 1.2, 1)
)
ggsave(
  "fig1c_human_tissue.pdf",
  fig1c_human,
  width = unit(8, "cm"),
  height = unit(3, "cm")
)
...
lgd_squre <- ComplexHeatmap::Legend(
  at = names(stg_cols),
  type = "grid",
...
pdf(glue("fig1d_mouse_plot1cell_all_genes.pdf"),
    width = 8,
    height = 8)
plot_circlize(
  circ_dat,
...
add_track(circ_dat,
          group = "stage",
          colors = stg_cols[order(names(stg_cols))],
          track_num = 2)
draw(
  lgd_squre,
  x = unit(40, "mm"),
...
pdf(glue("fig1d_mouse_plot1cell_mtb_genes.pdf"),
    width = 8,
    height = 8)
plot_circlize(
  circ_dat,
...
pdf(glue("fig1c_plot1cell_all_mz_mrg.pdf"),
    width = 5.5,
    height = 5.5)
plot_circlize(
  circ_dat,
...
add_track(circ_dat,
          group = "stage",
          colors = stg_cols[sort(unique(circ_dat$stage))],
          track_num = 2)
draw(
  lgd_squre,
  x = unit(25, "mm"),
...
pdf(glue("fig1d_human_plot1cell_all_genes.pdf"),
    width = 8,
    height = 8)
plot_circlize(
  circ_dat,
...
add_track(circ_dat,
          group = "stage",
          colors = stg_cols[sort(names(stg_cols))],
          track_num = 2)
...
pdf(
  glue("fig1d_human_plot1cell_mtb_genes_merged.pdf"),
  width = 8,
  height = 8
)
plot_circlize(
...
pdf(
  glue("fig1d_human_plot1cell_all_mz_mrg.pdf"),
  width = 5.5,
  height = 5.5
)
plot_circlize(
