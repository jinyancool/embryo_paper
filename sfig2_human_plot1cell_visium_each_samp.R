...
for (idx in 1:length(seu_new2_lst)) {
  ...
  pdf(
    glue(
      "figS2c_plot1cell_all_genes_{names(seu_new2_lst)[idx]}.pdf"
    ),
    width = 8,
    height = 8
  )
  ...
  add_track(circ_dat,
            group = "stage",
            colors = stg_cols[unique(seu_new2_lst[[idx]]$stage)],
            track_num = 2)
  draw(
    lgd_squre,
    x = unit(40, "mm"),
...
for (idx in 1:length(seu_new3_lst)) {
  ...
  pdf(
    glue(
      "figS3_human_plot1cell_mtb_genes_{unique(seu_new3_lst[[idx]]$stage)}.pdf"
    ),
    width = 8,
    height = 8
  )
  ...
  add_track(circ_dat,
            group = "stage",
            colors = stg_cols[unique(seu_new3_lst[[idx]]$stage)],
            track_num = 2)
...
for (idx in 1:length(seu_lst)) {
  ...
  pdf(
    glue(
      "figS2c_human_plot1cell_mtb_genes_{names(seu_lst)[idx]}_v2.pdf"
    ),
    width = 8,
    height = 8
  )
  ...
  add_track(circ_dat,
            group = "stage",
            colors = stg_cols[unique(seu_lst[[idx]]$stage)],
            track_num = 2)
