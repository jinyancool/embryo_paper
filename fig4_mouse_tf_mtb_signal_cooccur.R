...
write_csv(mouseobj_full_frame,
          "../rds/fig4a_tf_mouseobj_full_frame.csv")
csv_fn1 <-
...
parallel::mclapply(setis, function(seti) {
  ...
  ggsave(
    glue("./fig4a_mouse_tf_{seti}_v250322.pdf"),
    width = 7,
    height = 3
  )
}, mc.cores = 3)
...
parallel::mclapply(setis, function(seti) {
  p <- humanobj_full_frame |>
    ...
    Seurat::DarkTheme() + NoGrid() + NoAxes()
  ggsave(
    glue("./fig4b_human_tf_{seti}_v250322.pdf"),
    width = 7,
    height = 3,
    unit = "in"
  )
...
tbl1 <- tibble(tf = "Hnf4a",
               metabolic = c("Purine metabolism", "Pyrimidine metabolism"))
tbl2 <- tibble(
  tf = "Sox2",
  metabolic = c(
...
plst3 <- lapply(1:nrow(comp_df), \(idx) {
  ...
  sp2 <- ggplot2::ggplot() + ggplot2::geom_point(
    data = df4p,
    aes(x = x, y = y, color = feat2),
    show.legend = F,
    alpha = 1,
    size = .2
  ) + labs(color = feat1, color = "") +
...
pdf(glue("fig4gi_e115_tf_mz_co_expr.pdf"),
    width = 8,
    height = 3)
print(plst3)
dev.off()
