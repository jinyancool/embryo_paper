...
for (idx in 1:length(mz_mrg_lst)) {
  ...
  pdf(
    glue("figS3_plot1cell_all_mz_{names(mz_mrg_lst)[idx]}.pdf"),
    width = 5.5,
    height = 5.5
  )
  plot_circlize(
  ...
  add_track(circ_dat,
            group = "stage",
            colors = stg_cols[names(mz_mrg_lst)[idx]],
            track_num = 2)
...
for (idx in c("yao1", "yao2", "yao5")) {
  ...
  pdf(
    glue("figS3c_plot1cell_all_mz_{unique(obj1$stage)}.pdf"),
    width = 5.5,
    height = 5.5
  )
  plot_circlize(
  ...
  add_track(circ_dat,
            group = "stage",
            colors = stg_cols[unique(obj1$stage)],
            track_num = 2)
