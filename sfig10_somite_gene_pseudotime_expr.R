...
p1 <- monocle::plot_cell_trajectory(cds_vld, cell_size = .2, color_by = "State") +
  ...
  theme(legend.key.size = unit(3, "mm"),
        axis.text = element_text(color = "black")) +
  coord_fixed()
ggsave("sfig10d_ddrtree_state.pdf",
       p1,
       width = 3,
       height = 2)
...
pdf("sfig10e_e11.5_somite_heatmap1.pdf",
    width = 6,
    height = 8)
...
ggsave("sfig10f_kegg_mtb_enrich_clust4.pdf",
       pt1,
       width = 5,
       height = 3)
...
celltype_isoheight_plot <- function(.data,
                                    ...
                                    col_isoheight = "white",
                                    col_white_ratio = .25,
                                    cols_fill_isoheight = c(rep("white", round(100 * .25)), colorRampPalette(brewer.pal(5, "YlOrRd")[2:5])(round(100 * (1 - .25)))),
                                    size_bg = 0.2,
                                    size_top = size_bg) {
  df <- .data@meta.data |>
  ...
  p <- p +
    ggplot2::coord_fixed(
      ratio = 1,
      xlim = c(1.05 * x_min, 0.9 * x_max),
      ylim = c(0.9 * y_min, 1.05 * y_max),
      expand = F
    ) +
    theme_classic(base_size = 8) +
    theme(
...
pdf("sfig10gj_e11.5_somite_mtb_genes.pdf",
    width = 2,
    height = 3)
print(feats_lst)
dev.off()
