...
feat1 = Seurat::SpatialFeaturePlot(
  seu1_brain2,
  features = "Pseudotime",
  pt.size.factor = 1.3,
  image.alpha = 1
...
ggsave("fig6b_gene_pseudotime_spatial.pdf",
       feat1,
       width = 4,
       height = 2)
rds_fn2 <-
...
p_traj1 <- plot_cell_trajectory(cds_vld_gene, color_by = "Pseudotime", cell_size = 5e-1) +
  ...
  theme(legend.position = "right",
        legend.key.size = unit(3, "mm")) + ggplot2::coord_fixed()
ggsave(
  glue::glue("fig6b_pseudotime_monocle2_gene.pdf"),
  p_traj1,
  width = 4,
  height = 2
)
...
p_traj2 <- plot_cell_trajectory(cds_vld_mz, color_by = "Pseudotime", cell_size = 5e-1) +
  ...
  theme(legend.position = "right",
        legend.key.size = unit(3, "mm")) + ggplot2::coord_fixed()
ggsave(
  glue::glue("fig6b_pseudotime_monocle2_mz.pdf"),
  p_traj2,
  width = 4,
  height = 2
)
...
feat2 = Seurat::SpatialFeaturePlot(mz_brain, features = "Pseudotime", pt.size.factor = 1.3) &
  my_theme1 & coord_fixed() & viridis::scale_fill_viridis()
ggsave("fig6b_mz_pseudotime_spatial.pdf",
       feat2,
       width = 4,
       height = 2)
...
comp_df <- tibble(
  signaling = "Notch signaling pathway",
  metabolic = c(
...
for (samp in c("E115", "E135")) {
  ...
  plst3[[samp]] <- lapply(1:nrow(comp_df), \(idx) {
    ...
    df4p <- cord1 |> dplyr::select(all_of(c("x", "y", feat1, feat2))) |>
      ...
      mutate(top = case_when((feat1 > quantile(.$feat1, .85)) &
                               (feat2 > quantile(.$feat2, .85)) ~ "top 15%", TRUE ~ "no"))
    p <- ggplot2::ggplot() +
      ggplot2::geom_point(
        data = df4p,
        aes(x = x, y = y, color = feat1),
        show.legend = F,
        alpha = .5,
        size = .2
      ) + labs(color = feat1) +
      ...
      ggnewscale::new_scale_color() +
      geom_point(
        data = dplyr::filter(df4p, top == "top 15%"),
...
fig6g <- (plst3[[1]][[1]] |
            plst3[[2]][[1]]) / (plst3[[1]][[2]] |
                                  plst3[[2]][[2]]) / (plst3[[1]][[3]] | plst3[[2]][[3]])
pdf(
  glue("fig6g_e115_e135_signal_mz_co_expr.pdf"),
  width = 7,
  height = 11
)
print(fig6g)
