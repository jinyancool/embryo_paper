...
for (samp in samples) {
  csv_fn2 <-
    glue(
      "/cluster/home/ztao_jh/projects/embryo/analysis/zhangjing/human/rnaseq/pyscenic/mouse/{samp}/{samp}_SCENIC.csv"
    )
...
for (samp in samples) {
  csv_fn2 <-
    glue(
      "/cluster/home/ztao_jh/projects/embryo/analysis/zhangjing/human/rnaseq/pyscenic/human/{samp}/{samp}_SCENIC.csv"
    )
  tf_score <- read_csv(csv_fn2) |> mutate(barcode = str_sub(Cell, end = -3))
